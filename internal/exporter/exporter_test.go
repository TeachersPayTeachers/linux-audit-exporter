...
func TestDescribe(t *testing.T) {
	...
	if len(desc) != 7 {
		t.Errorf("Got %d metric descriptions; want 7.", len(desc))
	}
	...
	if desc[1].String() != expect {
		t.Errorf("Got %s metric[1] description; want %s.", desc[0].String(), expect)
	}
	...
	if desc[2].String() != expect {
		t.Errorf("Got %s metric[2] description; want %s.", desc[0].String(), expect)
	}
	...
	if desc[3].String() != expect {
		t.Errorf("Got %s metric[3] description; want %s.", desc[0].String(), expect)
	}
	...
	if desc[4].String() != expect {
		t.Errorf("Got %s metric[4] description; want %s.", desc[0].String(), expect)
	}
	...
	if desc[5].String() != expect {
		t.Errorf("Got %s metric[5] description; want %s.", desc[0].String(), expect)
	}
	...
	if desc[6].String() != expect {
		t.Errorf("Got %s metric[6] description; want %s.", desc[0].String(), expect)
	}
}
...
func TestCollect(t *testing.T) {
	...
	if len(metric) != 7 {
		t.Errorf("Got %d metrics; want 7.", len(metric))
	}
	...
	if metric[3].Desc() != e.enabledDesc {
		t.Errorf("Got metric[3].Desc() %s; want %s.", metric[3].Desc().String(), e.enabledDesc.String())
	}
	...
	if o.Gauge.GetValue() != float64(status.Enabled) {
		t.Errorf("Got metric[3] value %f; wanted %d.", o.Gauge.GetValue(), status.Enabled)
	}
	...
	if metric[4].Desc() != e.failureDesc {
		t.Errorf("Got metric[4].Desc() %s; want %s.", metric[4].Desc().String(), e.failureDesc.String())
	}
	...
	if o.Counter.GetValue() != float64(status.Failure) {
		t.Errorf("Got metric[4] value %f; wanted %d.", o.Counter.GetValue(), status.Failure)
	}
	...
	if metric[5].Desc() != e.lostDesc {
		t.Errorf("Got metric[5].Desc() %s; want %s.", metric[5].Desc().String(), e.lostDesc.String())
	}
	...
	if o.Counter.GetValue() != float64(status.Lost) {
		t.Errorf("Got metric[5] value %f; wanted %d.", o.Gauge.GetValue(), status.Lost)
	}
	...
	if metric[6].Desc() != e.rateLimitDesc {
		t.Errorf("Got metric[6].Desc() %s; want %s.", metric[6].Desc().String(), e.rateLimitDesc.String())
	}
	...
	if o.Gauge.GetValue() != float64(status.RateLimit) {
		t.Errorf("Got metric[6] value %f; wanted %d.", o.Gauge.GetValue(), status.RateLimit)
	}
}
